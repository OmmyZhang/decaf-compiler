...
%token VOID   BOOL  INT   STRING   COMPLEX
%token CLASS
%token NULL   EXTENDS     THIS   SUPER  WHILE   FOR   
%token IF     ELSE        CASE  DEFAULT RETURN   BREAK   NEW
%token DO     OD    SEPA
...
DoStmt          :   DO DoBranchList OD
                {
                    $$.stmt = new Tree.DoStmt($2.dblist, $1.loc);
                }
                ;
...
DoBranchList   :    DoBranchList SEPA DoBranch
                ...
                |   DoBranch
                {
                    $$.dblist = new ArrayList<Tree.DoBranch>();
                    $$.dblist.add($1.db);
                }
                ;                       
...
DoBranch        :   Expr ':' Stmt
                {
                    $$.db = new Tree.DoBranch($1.expr, $3.stmt, $1.loc);
                }
                ;
...
Expr            :	LValue
                ...
                |	CASE '(' Expr ')' '{' CaseExprList DefaultExpr '}'
                	{
                		$$.expr = new Tree.Case($3.expr , $6.caselist , $7.expr , $1.loc);
                	}
                |   DCOPY '(' Expr ')'
                    {
...
CaseExprList	:	CaseExprList ACaseExpr
					{
						   $$.caselist.add($2.acase);
					}
				|	/* empty */
					{
					    $$.caselist = new ArrayList<ACaseExpr>();
					}
				;
					
ACaseExpr		:	Constant ':' Expr ';'
					{
					    $$.acase = new Tree.ACaseExpr($1.expr, $3.expr, $1.loc);
					}
				;
