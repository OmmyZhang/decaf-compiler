...
public class TypeCheck extends Tree.Visitor {
	...
	private void checkCallExpr(Tree.CallExpr callExpr, Symbol f) {
		...
		if (callExpr.receiver != null && callExpr.receiver.tag == Tree.SUPEREXPR)
			receiverType = ((ClassType)receiverType).getParentType();
		if (f == null) {
	...
	public void visitCallExpr(Tree.CallExpr callExpr) {
		...
		callExpr.receiver.accept(this);
		//System.out.println(callExpr.method + "_" + callExpr.receiver);
		...
		boolean isSuper = callExpr.receiver.tag == Tree.SUPEREXPR;

		if (isSuper && ((ClassType) callExpr.receiver.type).getParentType()==null)
		{
			issueError(new SuperNoParent(callExpr.getLocation(), callExpr.receiver.type.toString()));
			callExpr.type = BaseType.ERROR;
			return;
		}
		//System.out.println("par: " + ((ClassType) callExpr.receiver.type).getParentType());

		ClassScope cs = isSuper? 
					((ClassType) callExpr.receiver.type).getClassScope().getParentScope()
				:	((ClassType) callExpr.receiver.type).getClassScope();
		checkCallExpr(callExpr, cs.lookupVisible(callExpr.method));
	}
	...
	public void visitDCopy(Tree.DCopy dcopy)
	{
		...
		{
			issueError(new CopyNotClassError(dcopy.getLocation(),
						dcopy.expr.type.toString()));
	...
	public void visitSCopy(Tree.SCopy scopy)
	{
		...
		{
			issueError(new CopyNotClassError(scopy.expr.getLocation(),
	...
	public void visitSuperExpr(Tree.SuperExpr superExpr)
	{
		...
		else 
		{
			superExpr.type = ((ClassScope) table.lookForScope(Scope.Kind.CLASS))
					.getOwner().getType();
//			System.out.println("SUPER: "+superExpr.type);
		}
	}
	...
	public void visitIdent(Tree.Ident ident) {
		//System.out.println("Id: "+ident.owner+" _ "+ident.name+" _ "+ident.symbol);
		if (ident.owner == null) {
			...
			if (!ident.owner.type.equal(BaseType.ERROR)) {
				...
				if (ident.owner.isClass || !ident.owner.type.isClassType()) {
					...
					if (v == null) {
						//System.out.println("Not found: " + ident.owner.type + "__" + ident.name);
	...
	public void visitAssign(Tree.Assign assign) {
		...
		//System.out.println("assign: "+assign.expr);
		if (assign.expr.tag == Tree.SCOPY || assign.expr.tag == Tree.DCOPY)
		{
			if (!assign.left.type.equal(BaseType.ERROR)
				...
				issueError(new CopyNotSameError(assign.getLocation(),
	...
	{	
		...
		c.type = c.defaultExpr.type;
		ok = !c.type.equal(BaseType.ERROR);

		//System.out.println("CASE!! "+c.caseList);
		if (!c.condition.type.equal(BaseType.ERROR)
				&& !c.condition.type.equal(BaseType.INT))
		{
			ok = false;
			issueError(new BadCaseExprError(c.condition.getLocation(),
						c.condition.type.toString()));
		}
		for(Tree.ACaseExpr ac:c.caseList)
		{
			ac.caseCont.accept(this);
			if(ac.caseCont.type.equal(BaseType.ERROR))
				continue;
			if(!ac.caseCont.type.equal(BaseType.INT))
			{
				ok = false;
				issueError(new BadCaseExprError(ac.getLocation(),
							ac.caseCont.type.toString()));
			}
			else
			{
					int ci = (Integer)(((Tree.Literal)ac.caseCont).value);
					if (cs.contains(ci))
						issueError(new CaseConditionNotUniqueError(ac.getLocation()));
					else
						cs.add(ci);
		...
		{
			ac.expr.accept(this);
			if (ac.expr.type.equal(BaseType.ERROR))
			{
				ok = false;
				continue;
			}
			if (c.type.equal(BaseType.ERROR))
				c.type = ac.expr.type;
			else
				if (!c.type.equal(ac.expr.type))
				{
					ok = false;
					issueError(new CaseTypeDifferentError(ac.getLocation(), ac.expr.type.toString(), c.type.toString()));

				}
		}
		if(!ok)
			c.type = BaseType.ERROR;
	}
	...
	{
		breaks.add(dostmt);
		for(Tree.DoBranch db:dostmt.doBranchList)
			db.accept(this);
		breaks.pop();
	}
	
	@Override
	public void visitDoBranch(Tree.DoBranch db)
	{
		//db.condition.accept(this);
		checkTestExpr(db.condition);
		db.stmt.accept(this);

		if (!db.condition.type.equal(BaseType.ERROR)
				&& !db.condition.type.equal(BaseType.BOOL))
			issueError(new DoConditionNotBoolError(db.condition.getLocation(),
						db.condition.type.toString()));
	}
...
//			}
		...
		case Tree.MUL:
		case Tree.DIV:
			compatible = (left.type.equal(BaseType.INT) || left.type.equal(BaseType.COMPLEX))
					&& (right.type.equal(BaseType.INT) || right.type.equal(BaseType.COMPLEX));
			returnType = right.type.equal(BaseType.INT) ? left.type : BaseType.COMPLEX;
			break;
		case Tree.GT:
		case Tree.GE:
		case Tree.LT:
